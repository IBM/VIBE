...
describe('ImportReviewTable', () => {
	it('shows allowed actions by analysis status', () => {
		...
		expect(within(newSelect).getAllByRole('option').map((option) => option.getAttribute('value'))).toEqual([
			'skip',
			'create_new'
		]);

		const conflictSelect = screen.getByLabelText('Resolution for agents Conflict');
		expect(within(conflictSelect).getAllByRole('option').map((option) => option.getAttribute('value'))).toEqual([
			'skip',
			'overwrite',
			'create_new'
		]);
		...
		expect(within(dependencySelect).getAllByRole('option').map((option) => option.getAttribute('value'))).toEqual([
			'skip'
		]);
